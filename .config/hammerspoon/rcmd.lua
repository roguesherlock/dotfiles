...
function rcmdWindowsChangedFocused(window, app, event)
	...
	rcmdWindowsAll = rcmdFilterArray(rcmdWFAll:getWindows(), function(w)
		return string.len(w:title()) > 0
	end)
...
function rcmdJSON(windows)
	...
	return hs.json.encode(rcmdMap(windows, function(w)
		return rcmdEncodeWindow(w, focusedID)
	end))
end
...
function rcmdWindowsChanged(window, app, event)
	...
	rcmdWindows[app] = rcmdFilterArray(wfilter:getWindows(), function(w)
		return string.len(w:title()) > 0
	end)
end
...
function getWindowFilter(params)
	...
	local filters = {
		allowRoles = { "AXFloatingWindow", "AXStandardWindow", "AXSystemDialog", "AXSystemFloatingWindow", "AXDialog" },
	}
...
function rcmdCallbackWS(message)
	...
	if not params.token or params.token ~= "e1c9328256ac34747b3ff35dd9e29b0f6acb3307" then
		return "Unauthorized"
	end
	...
	elseif params.cmd == "minimizeall" then
		...
		local alreadyMinimized = hs.fnutils.every(windows, function(w)
	...
	elseif params.cmd == "launch" then
		if params.app then
			if params.args then
				local t = hs.task.new(params.app, function(exitCode, stdOut, stdErr)
					if params.name then
						io.open(RCMD_LOG_TMP_DIR .. "/" .. params.name, "w")
							:write(tostring(exitCode) .. "\n" .. stdOut .. "\n" .. stdErr)
							:close()
					end
				end, params.args)
				t:start()
...
rcmdWFAll = hs.window.filter.new({
	default = {
		allowRoles = { "AXFloatingWindow", "AXStandardWindow", "AXSystemDialog", "AXSystemFloatingWindow", "AXDialog" },
	},
})
